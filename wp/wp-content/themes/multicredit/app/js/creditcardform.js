...
function instantValidation(field) {
	  ...
	  if (shouldBeValidated(field)) {
        ...
        if (field.classList.contains("birth")) {
            var successmessagen = field.parentNode.parentNode.querySelector('.form-field-valid');
            var errormessagen = field.parentNode.parentNode.querySelector('.form-field-invalid');
            var totalfields = groupfields.length;
          if (field.value) {
            field.removeAttribute("aria-invalid");
            field.classList.add("is-valid");
            errormessagen.classList.add("visible");
            var totalgroup = document.querySelectorAll('.birth.is-valid').length;
            // console.log(totalgroup);
              if(totalgroup==totalfields) {
                successmessagen.classList.add("visible");
                errormessagen.classList.remove("visible");
              }
            } else {
                field.setAttribute("aria-invalid", "true");
                field.classList.remove("is-valid");
                successmessagen.classList.remove("visible");
                errormessagen.classList.add("visible");
            }
        }
...
// Jeremy - Start
document.getElementById("cash_form").addEventListener("submit", function(evt){
	evt.preventDefault();
	submitForm();
	return false;
});
...
function submitForm() {
	$('#cash_form #localite').removeAttr('disabled')
	$("#cash_form #submit").attr('disabled', 'disabled');
	$("#cash_form #submit").html('Chargement...');

	var naissanceDate = $('#cash_form #naissance-yyyy').val() + '-' + $('#cash_form #naissance-mm').val() + '-' + $('#cash_form #naissance-dd').val()
	var dataToSubmit = $('#cash_form').serialize() + "&client_naissance=" + naissanceDate;
	var xhr = new XMLHttpRequest();
	xhr.open("POST", 'https://crm.cashflex.ch/curl/formulaires/prives/submit.php', true);
	xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	xhr.onreadystatechange = function() {
	    if (xhr.readyState == XMLHttpRequest.DONE) {
	        $('#cash_form #localite').attr('disabled', 'disabled');
		    $("#cash_form").hide();
